...
public class Graph {
	...
	/**
	 * Add an edge to this graph. If an edge from source to dest already exists
	 * then this edge replaces the old edge. Creates vertices for source and /
	 * or dest if they are not already present. <br>
	...
	/**
	 * Add a vertex with the given name to this Graph. The new vertex has no
	 * edges initially. <br>
	 * pre: name != null
	 * 
	 ...
	 *                  unweighted shortest paths from the give vertex to all
			...
			for (Edge e : current.adjacent) {
				...
				if (neighberNode.weightedCostFromStartVertex == INFINITY) {
					...
					neighberNode.weightedCostFromStartVertex = current.weightedCostFromStartVertex
							+ 1;
					neighberNode.numEdgesFromStartVertex = current.numEdgesFromStartVertex
							+ 1;
	...
	private Queue<Vertex> prepForFindingUnweightedShortestPaths(
			String startName) {
	 ...
	 * 
	 * @param startName The starting vertex. This method will find all the
	 *                  weighted shortest paths from the given vertex to all
	 *                  other vertices in the graph.
	 */
	public void dijkstra(String startName) {
		...
		PriorityQueue<Path> queue = prepForDijkstra(startName);
		...
		while (!queue.isEmpty()) {
			...
			if (current.scratch == 0) {
				...
				for (Edge edge : current.adjacent) {
					...
					double cost = current.weightedCostFromStartVertex + edge.cost;

					if (cost < adjacent.weightedCostFromStartVertex) {
						adjacent.weightedCostFromStartVertex = cost;
						adjacent.numEdgesFromStartVertex = current.numEdgesFromStartVertex + 1;
						adjacent.prev = current;
					}

					queue.add(new Path(adjacent, cost));
				}
			}
		}
	}

	// Set up PriorityQueue and instance variables to find
	// weighted shortest path algorithm from startName vertex
	// to every other Vertex we can reach.
	private PriorityQueue<Path> prepForDijkstra(String startName) {
		for (String name : vertices.keySet()) {
			vertices.get(name).weightedCostFromStartVertex = INFINITY;
		}
		currentStartVertexName = startName;
		clearAll();
		Vertex start = vertices.get(startName);
	 ...
	 * 1. The total number of vertices (not including itself) it is connected
	 * to. In other words, for a given vertex, the number of other vertices for
	 * which a path exists. <br>
	 ...
	 * 3. The sum of the weighted path lengths in the all the shortest paths
	 * from the vertex to the vertices it connects to. <br>
	 * <br>
	 * Note, if the parameter weighted is false, 2 and 3 shall equal each other.
	 *
	 * <br>
	 * <br>
	 * This method also determines the shortest path with the greatest distance.
	 * The two most distant vertices in the Graph are those with the Longest
	 * shortest path between them. The longest path is based on the number of
	 * edges in the path if weighted == false and the unweighted shortest path
	 * algorithm is being used. The longest path is based on the highest cost
	 * shortest path if weighted == true and Dijkstra is used.
	 *
	 * <br>
	 * <br>
	 * if weighted == true, use dijkstra, otherwise, use unwieghted shortest
	 * path
	 *
	 * <br>
	 * <br>
	 * After this method is called the getAllPaths, getDiamter, and get longest
	 * path methods may be called.
	 *
	 * @param weighted If weighted == true use dijkstra's algorithm otherwise
	 *                 use the unweighted shortest path algorithm. (Ignore any
	 *                 weights for edges. All edge weights considered to be 1.)
	 */
	public void findAllPaths(boolean weighted) {
		for (String name : vertices.keySet()) {
			// find all the paths from cur to every other vertex
			if (weighted) {
				dijkstra(name);
			} else {
				findUnweightedShortestPath(name);
			}
			Vertex cur = vertices.get(name);
			cur.numVertexConnected = 0;
			cur.totalUnweightedPathLength = 0;
			cur.totalWeightedPathLength = 0;

			// ready to process current vertex (name)
			for (String other : vertices.keySet()) {
				List<String> paths = findPath(other);

				// if a path exists between cur and other vertex
				if (!other.equals(name) && paths.size() != 0) {
					cur.numVertexConnected++;
					
					cur.totalWeightedPathLength += getPath(
							other).weightedCostOfPath;
					cur.totalUnweightedPathLength += getPath(other)
							.getNumVerticesInPath();

					// the longest shortest path
					if (paths.size() > longest.getNumVerticesInPath()) {
						longest = getPath(other);
					}
				}
			}
		}
	 ...
	 * null pre: dest refers to a vertex in this Graph pre: A path exists from
	 * the current start vertex to dest. post: return a Path object with the
	 * Path from the current start vertex to the Vertex specified by dest.
	 */
	private Path getPath(String dest) {
		...
		if (end.prev == null) {
			...
			throw new IllegalStateException("getPath(String)"
					+ " a path must exist from " + this.currentStartVertexName
					+ " to " + dest + ".");
		}
	...
	/**
	 * Get the number of edges of the shortest path from the current start
	 * vertex to the given destination vertex. If there is no path from the
	 * current start vertex to the given destination vertex, returns -1. <br>
	 * <br>
	 * pre: findUnweightedShortestPath or dijkstra called.
	 * <tt>containsVertex(dest) == true</tt>
	 * 
	 * @param dest the destination vertex.
	 * @return the number of edges from the current start vertex to the
	 *         destination vertex. returns -1 if no path exists.
	 */
	 ...
	 * vertex to the given destination vertex. If there is no path from the
	 * current start vertex to the given destination vertex -1 is returned. <br>
	 * <br>
	 * pre: findUnweightedShortestPath or dijkstra called.
	 * <tt>containsVertex(dest) == true</tt>
	 * 
	 * @param dest
	 * @return the total cost of the shortest path from the current start vertex
	 *         to the destination vertex. returns -1 if no path exists.
	 */
	...
	/**
	 * Get all path statistics for all vertices in this graph that are connected
	 * to one or more other vertices. Vertices are ordered by centrality. For
	 * this method the most central nodes is the one connected (path exists) to
	 * the largest number of other vertices. Ties based on number of vertices
	 * connected (expected to occur in many graphs) are broken based on the
	 * average cost (weight) of the paths. The lower the average cost, the more
	 * central a vertex. <br>
		...
		for (Vertex v : vertices.values()) {
			if (v.numVertexConnected > 0) {
				AllPathsInfo temp = new AllPathsInfo(v.name,
						v.numVertexConnected, v.totalWeightedPathLength);
	 ...
	 * @return true if a vertex with name is present in this Graph, false
	 *         otherwise.
	 */
	...
	/**
	 * Alternative to printPath that returns an List containing the path from
	 * the current start vertex to the vertex named destName. If no path exists
	 * an empty List is returned. <br>
	 * pre: destName != null, containsVertex(destName) == true, the starting
	 * vertex has been set by calling findUnweigthedShortesPath or dijkstra.
	 * 
	 * @param destName The destination vertex
	 * @return A list with the path from the current start vertex to the vertex
	 *         with destName. start vertex will be at index 0 and destName will
	 *         be at index list.size() - 1 unless there is no path from the
	 *         start vertex to destName in which case an empty list is returned.
	 */
	...
	/**
	 * Print the path from the current start vertex to the vertex with name
	 * destName <br>
	 * pre: destName != null, containsVertex(destName) == true, the startNode
	...
	/**
	 * Return the number of edges in the longest shortest path in this Graph.
	 * <br>
	 * pre: findAllPaths has been called.
	 * 
	...
	/**
	 * Return the cost of the longest shortest path in this Graph. Note, the
	 * cost of the longest shortest path may differ than the number of edges in
	 * the path. (returned by getDiameter) <br>
	 * pre: findAllPaths has been called.
	 * 
	...
	/**
	 * Prints out the name of all vertices in the Graph. Not expected to use on
	 * A12
	 */
	public void showAll() {
		for (Vertex v : vertices.values()) {
	...
	private static class Vertex {
		...
		// Sum of all unweighted shortest paths from this vertex to other
		// vertices.
		// This variable shall be updated in the findAllPaths method.
		private double totalUnweightedPathLength;

		// Sum of all weighted shortest paths from this vertex to other
		// vertices.
		// This variable shall be updated in the findAllPaths method.
		...
		public String toString() {
			return "{" + name + ", " + ", connected to: " + numVertexConnected
					+ "adjacent: " + adjacent + "}";
		}
	...
	private static class Path implements Comparable<Path> {
		...
		private List<Vertex> verticesInPath; // not necessary for temporary
												// paths in Dijkstra's
