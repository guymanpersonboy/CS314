...
public class FootballRanker {
	...
	// to adjust output
	private static final DecimalFormat ourFormatter = new DecimalFormat(
			"0.0000");
	...
	/**
	 * Create a new FootballRanker based on the given files. Format of file is:
	 * <br>
	 * Date,Home Team Name,Home Team Points,Away Team Name,Away Team
	 * Points,Location if neutral. <br>
	...
	/**
	 * Calculate top teams based on average path length of weighted shortest
	 * paths. This takes into account how easily teams won their games.
	 * 
	 * @param showResults if this is true output results to standard output
	 * @return the root mean square error of the top 25 teams based on this
	 *         calculation rounded to 1 decimal place.
	...
	/**
	 * Calculate top teams based on average path length of weighted shortest
	 * paths with an adjustment for the teams winning percentage.
	 * 
	 * @param showResults if this is true output results to standard output
	 * @return the root mean square error of the top 25 teams based on this
	 *         calculation rounded to 1 decimal place.
	 ...
	 * results of poll data. return the root mean square error of difference
	 * between acutal poll results and predicted results from graph. If
	 * showResults is true print results to standard output.
	 */
	private double printRootMeanSquareError(List<String> humanRanks,
		...
		for (AllPathsInfo pred : paths) {
			predicted.add(pred.getName());
		}

		int rank = 1;
		for (String team : humanRanks) {
			double graphPrediction = predicted.indexOf(team) + 1;
			// a team is not ranked in our predictions
			if (graphPrediction == 0) {
				graphPrediction = humanRanks.size() + 1;
			}

			final double difference = rank - graphPrediction;
			rootMeanSquare += difference * difference;
			rank++;
		}

		rootMeanSquare = Math.sqrt(rootMeanSquare / humanRanks.size());
		if (showResults) {
			printData(humanRanks, predicted,
					ourFormatter.format(rootMeanSquare));
		}
		DecimalFormat resultFormatter = new DecimalFormat("0.0");

		return Double.parseDouble(resultFormatter.format(rootMeanSquare));
	}

	// prints the predicted vs. actual results
	// pre: params != null, handled in calling method
	// post: print out the top 25 ranked teams, their predicted rank, and the
	// root mean square error.
	private void printData(List<String> humanRanks, ArrayList<String> predicted,
			String printableError) {
		System.out.println("\n\n ***** PREDICTED VS. ACTUAL RESULTS *****");

		int rank = 1;
		for (String team : humanRanks) {
			System.out.printf("%-" + PADDING + "s", team);

			System.out.println(" - actual rank: " + rank + " predicted rank: "
					+ (predicted.indexOf(team) + 1));
			rank++;
		}
		System.out.println("Root Mean Square Error: " + printableError);
	}
	...
	public void processRequests() {
		...
		do {
			...
			if (teamsAndResults.containsVertex(start)) {
				...
				} while (another(in, "Do you want to find another path from "
						+ start + "?"));
			} else {
				System.out.println("\n" + start + " is not a team");
			}
		} while (another(in,
				"Do you want to look for more paths between teams?"));
	}
	...
	private TreeSet<AllPathsInfo> rankByScoreAdjustWinPercentage(
			Set<AllPathsInfo> paths) {
