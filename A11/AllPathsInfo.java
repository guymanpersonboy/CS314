...
/**
 * This class is used to store information about a vertex and its paths to other
 * vertices. Paths are not stored individually. Instead aggregate statistics for
 * the paths are stored including the name of the vertex, the number of paths
 * (which equals the number of other vertices the vertex is connected to) and
 * the total cost of all the paths.
 */
...
public class AllPathsInfo implements Comparable<AllPathsInfo> {
	...
	/**
	 * Create a new AllPathsInfo object. <br>
	 * pre: st != null, ps > 0
	 * 
	...
	public AllPathsInfo(String st, int ps, double total) {
		if (st == null) {
			throw new IllegalArgumentException(
					"The vertex name may not be null. " + st);
		} else if (ps <= 0) {
			throw new IllegalArgumentException(
					"The number of paths must be greater than 0. " + ps);
		}
		vertexName = st;
		numPaths = ps;
		totalCost = total;
	...
	public int compareTo(AllPathsInfo other) {
		...
		if (result == 0) {
			result = (aveCost < other.aveCost) ? -1
					: (aveCost == other.aveCost) ? 0 : 1;
		}
		if (result == 0) {
	...
	public String toString() {
		return "Name: " + String.format("%1$-" + PADDING + "s", vertexName)
	...
	public static AllPathsInfo makeCopy(AllPathsInfo org) {
		if (org == null) {
			throw new IllegalArgumentException(
					"The parameter org may " + "not be null");

		}
		AllPathsInfo result = new AllPathsInfo(org.vertexName, org.numPaths,
				org.totalCost);
		return result;
	}

	/**
	 * Get the number of paths from this vertex to other vertices. This should
	 * be the number of shortest paths.
	 * 
